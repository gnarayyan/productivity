...
class ${className}RemoteDataSourceImpl
  ...
  @override
  Future<Either<CustomException, ${className}Model>> update(String id, ${className}Model $featureName) =>
      modify(id: id, fromMap: ${className}Model.fromMap, data: ${className.toLowerCase()}.toMap());
...
class ${className}RepositoryImpl implements ${className}Repository {
  ...
  @override
  Future<Result<List<$className>>> get${className}s() async {
    try {
  ...
  @override
  Future<Result<$className>> get${className}ById(String id) async {
    try {
      final remoteData = await remoteDataSource.get${className}ById(id);
      return Success(remoteData);
    } on ServerException {
      return const Error(ServerFailure());
    } on NetworkException {
  ...
  @override
  Future<Result<$className>> create$className($className $featureName) async {
    try {
      final ${featureName}Model = ${className}Model.fromEntity($featureName);
      final remoteData = await remoteDataSource.create$className(${featureName}Model);
      return Success(remoteData);
    } on ServerException {
      return const Error(ServerFailure());
    } on NetworkException {
  ...
  @override
  Future<Result<$className>> update$className(String id, $className $featureName) async {
    try {
      final ${featureName}Model = ${className}Model.fromEntity($featureName);
      final remoteData = await remoteDataSource.update$className(id, ${featureName}Model);
      return Success(remoteData);
    } on ServerException {
      return const Error(ServerFailure());
    } on NetworkException {
  ...
  @override
  Future<Result<void>> delete$className(String id) async {
    try {
...
''';
  ...
  Future<void> _generateUseCase(Directory basePath) async {
    final className = _toPascalCase(featureName);
    final file = File(
      '${basePath.path}/domain/usecases/get_${featureName}s.dart',
    );
...
import 'package:varosa_tech/features/$featureName/domain/entities/$featureName.dart';
import 'package:varosa_tech/features/$featureName/domain/repositories/${featureName}_repository.dart';
...
class Get${className}s implements ParamLessUseCase<List<$className>> {
  final ${className}Repository repository;

  Get${className}s(this.repository);

  @override
  Future<List<$className>> call() async {
    final result = await repository.get${className}s();
    return result.fold(
...
class Get${className}ById implements UseCase<$className, ${className}Params> {
  final ${className}Repository repository;

  Get${className}ById(this.repository);

  @override
  Future<$className> call(${className}Params params) async {
    final result = await repository.get${className}ById(params.id);
    return result.fold(
      (failure) => throw Exception(_mapFailureToMessage(failure)),
      (data) => data,
    );
...
class Create$className implements UseCase<$className, Create${className}Params> {
  ...
  @override
  Future<$className> call(Create${className}Params params) async {
    final result = await repository.create$className(params.$featureName);
    return result.fold(
      (failure) => throw Exception(_mapFailureToMessage(failure)),
      (data) => data,
    );
...
class Update$className implements UseCase<$className, Update${className}Params> {
  ...
  @override
  Future<$className> call(Update${className}Params params) async {
    final result = await repository.update$className(params.id, params.$featureName);
    return result.fold(
      (failure) => throw Exception(_mapFailureToMessage(failure)),
      (data) => data,
    );
...
class Delete$className implements VoidUseCase<${className}Params> {
  final ${className}Repository repository;

  Delete$className(this.repository);

  @override
  Future<void> call(${className}Params params) async {
    final result = await repository.delete$className(params.id);
    return result.fold(
      (failure) => throw Exception(_mapFailureToMessage(failure)),
      (data) => data,
    );
...
class Create${className}Params {
  final $className $featureName;

  Create${className}Params({required this.$featureName});
}

class Update${className}Params {
  final String id;
  final $className $featureName;

  Update${className}Params({required this.id, required this.$featureName});
}
''';
...
import '../../../core/usecases/usecase.dart';
import '../../../domain/usecases/get_${featureName}s.dart';
...
class ${className}Bloc extends Bloc<${className}Event, ${className}State> {
  ...
  Future<void> _onLoad${className}s(Load${className}s event, Emitter<${className}State> emit) async {
    ...
    try {
      final ${featureName}s = await get${className}s(const NoParams());
      emit(${className}sLoaded(${featureName}s: ${featureName}s));
    } catch (e) {
      emit(${className}Error(message: e.toString()));
    }
  }

  Future<void> _onLoad${className}ById(Load${className}ById event, Emitter<${className}State> emit) async {
    emit(${className}Loading());
    
    try {
      final $featureName = await get${className}ById(${className}Params(id: event.id));
      emit(${className}Loaded($featureName: $featureName));
    } catch (e) {
      emit(${className}Error(message: e.toString()));
    }
  }

  Future<void> _onCreate$className(Create${className}Event event, Emitter<${className}State> emit) async {
    emit(${className}Loading());
    
    try {
      await create$className(Create${className}Params($featureName: event.$featureName));
  ...
  Future<void> _onUpdate$className(Update${className}Event event, Emitter<${className}State> emit) async {
    ...
    try {
      await update$className(Update${className}Params(id: event.id, $featureName: event.$featureName));
  ...
  Future<void> _onDelete$className(Delete${className}Event event, Emitter<${className}State> emit) async {
    ...
    try {
      await delete$className(${className}Params(id: event.id));
  ...
  Future<void> _onRefresh${className}s(Refresh${className}s event, Emitter<${className}State> emit) async {
...
import '../blocs/$featureName/${featureName}_bloc.dart';
import '../blocs/$featureName/${featureName}_event.dart';
import '../blocs/$featureName/${featureName}_state.dart';
...
class _${className}ScreenState extends State<${className}Screen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${_toTitleCase(featureName)}s'),
        actions: [
          IconButton(
      ...
      body: BlocConsumer<${className}Bloc, ${className}State>(
        ...
        builder: (context, state) {
          ...
          return const Center(
            child: Text('Welcome to ${className}s'),
          );
        },
      ),
            ...
            TextButton(
              onPressed: () {
                ...
                context.read<${className}Bloc>().add(
                  Delete${className}Event(id: $featureName.${dataFields.first.name}.toString()),
                );
              },
